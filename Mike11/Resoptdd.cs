...
namespace bjd_model.Mike11
{
    public class AllResoptdd
    {
        ...
        public static Dictionary<string, List<DdInfo>> Get_ResGate_OptimizeDd(HydroModel hydromodel, List<Res_Level_Constraint> res_level_constraint)
        {
            ...
            //刷选出所有 最下游水库(没有下游水库也加入)
            List<string> res_down_list = Get_Down_ResList(model_instance);
            ...
            //获取各水库的最优闸站调度过程
            Dictionary<string, List<DdInfo>> res_optimize_ddinfo = Get_AllRes_XfddInfo(res_down_list, allres_gzdd_inflow, res_outflow_constraint, 
        ...
        public static Dictionary<string, List<DdInfo>> Get_AllRes_XfddInfo(List<string> res_down_list,Dictionary<string, Dictionary<DateTime, double>> allres_inflow,
            ...
            Dictionary<string, double> res_iteration_level, string model_instance)
        {
            ...
            //水库调蓄计算
            Dictionary<string, Dictionary<DateTime, double>> res_out_gc = new Dictionary<string, Dictionary<DateTime, double>>();
            for (int i = 0; i < res_down_list.Count; i++)
            {
                string res_name = res_down_list[i];

                //水库入流
                Dictionary<DateTime, double> res_inflow = allres_inflow[res_name];

                //调蓄计算
                Dictionary<DateTime, double> res_total_out = Get_ResXfddinfo(res_outflow_constraint, initial_levels, res_inflow, xx_levels,
                    res_iteration_level, model_instance, res_name, ref res_ddinfos);

                res_out_gc.Add(res_name, res_total_out);
            }
            return res_ddinfos;
        }
        ...
        public static List<string> Get_Down_ResList(string model_instance)
        {
            ...
            {
                string res_name = res_upreslist.ElementAt(i).Key;
                if (!have_downres_list.Contains(res_name)) res_down_list.Add(res_name);
            }
        ...
        private static List<string> Get_DownRes_list(string model_instance)
        {
            ...
            for (int i = 0; i < res_upreslist.Count; i++)
            {
                Dictionary<string, double> up_res1 = res_upreslist.ElementAt(i).Value;
                List<string> up_reslist = up_res1 == null?null: up_res1.Keys.ToList();
            ...
            string res_name, ref Dictionary<string, List<DdInfo>> res_ddinfos)
        {
            ...
            //该水库的其他建筑采用全关调度
            Dictionary<string, List<DdInfo>> res_oterstr_ddinfo = Get_Res_OtherClose_Ddinfo(res_name, str_ddinfo);
            ...
            //加入水库其他建筑关闭调度信息
            for (int j = 0; j < res_oterstr_ddinfo.Count; j++)
            {
                res_ddinfos.Add(res_oterstr_ddinfo.ElementAt(j).Key, res_oterstr_ddinfo.ElementAt(j).Value);
            }
            ...
            Dictionary<DateTime, double> res_inflow_dic, double constraint_level)
        {
            ...
            while (true)
            {
                ...
                if (Math.Abs(max_res_level - constraint_level) < 0.1 || kx_q > inflow_max * 0.95 || kx_q < 1.0 || n >= 50)
                {
                    break;
                }
            ...
            kx_q = Math.Round(kx_q * 1.2 / 10.0) * 10;
            return kx_q;
        }
        ...
        private static double Get_Res_OutQ3(double xx_level, double kx_q, double res_level, double now_in_q, double total_max_q)
        {
            ...
            else   //来流量小于设计控泄流量，分2种情况
            {
                ...
                else   //来流量小于设计控泄流量，且小于最大泄洪能力
                {
                    now_out_q = Math.Min(now_in_q, total_max_q);

                    //if (res_level > xx_level)
                    //{
                    //    //当库水位大于汛限水位时，按1.5倍来流量超泄，且得小于控泄流量和过流能力
                    //    now_out_q = Math.Min(Math.Min(now_in_q * 1.5, total_max_q),kx_q);
                    //}
                    //else
                    //{
